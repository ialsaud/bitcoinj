...
implements NewBestBlockListener, TransactionReceivedInBlockListener, PeerFilterProvider, KeyBag, TransactionBag, ReorganizeListener {
	...
	public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
		...
		try {
			...
			System.out.println(this.getIssuedReceiveAddresses());
	...
	public void completeTx(SendRequest req) throws InsufficientMoneyException {
		...
		try {
			...
			if (req.signInputs)
				signTransaction(req);
			System.out.println(req.tx.getInput(0));
			//   req.tx.getInput(0).getScriptSig().correctlySpends(req.tx, 0, req.tx.getOutput(0).getScriptPubKey());
			// Check size.
	...
	public void completeTxChecksum(SendRequest req, byte[] checksum) throws InsufficientMoneyException {
		lock.lock();
		try {
			checkArgument(!req.completed, "Given SendRequest has already been completed.");
			// Calculate the amount of value we need to import.
			...
			List<String> checksumArr= new ArrayList();
			String temp;
			for (TransactionOutput output : bestCoinSelection.gathered){
				req.tx.addInput(output);
				Script corOutput=output.getScriptPubKey();
				if(corOutput.isSentToChecksumContract()){
					List<ScriptChunk> chunks =corOutput.getChunks();
					temp=chunks.get(6).toString();
					checksumArr.add(temp.substring(13, temp.length()-1));
				}
			}
			if (req.emptyWallet) {
				final Coin feePerKb = req.feePerKb == null ? Coin.ZERO : req.feePerKb;
	...
	public void signTransaction(SendRequest req) {
		...
		try {
			...
			//TransactionInput txIn1 = tx.getInput(0);
			for (int i = 0; i < numInputs; i++) {
				...
				if(scriptPubKey.isTimeLockChecksumTx()){
					//found custom transaction. 
			...
			//            txIn1.getScriptSig().correctlySpends(tx, 0, txIn1.getConnectedOutput().getScriptPubKey());
	...
	public boolean canSignFor(Script script) {
		if (script.isSentToRawPubKey()) {
			...
			return key != null && (key.isEncrypted() || key.hasPrivKey());
		} else if(script.isSentToChecksumContract()){
			ECKey key = findKeyFromPubHash(script.getPubKeyHash());
			return key != null && (key.isEncrypted() || key.hasPrivKey());
		} else if(script.isTimeLockChecksumTx()) { 
