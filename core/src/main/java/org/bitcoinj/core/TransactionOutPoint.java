...
public class TransactionOutPoint extends ChildMessage {
     ...
     */
    @Nullable
    public RedeemData getConnectedRedeemData2(KeyBag keyBag, Script script) throws ScriptException {
    	
    	
    	TransactionOutput connectedOutput = getConnectedOutput();
    	checkNotNull(connectedOutput, "Input is not connected so cannot retrieve key");
    	Script connectedScript = connectedOutput.getScriptPubKey();
    	byte[] addressBytes = connectedScript.getPubKeyHash2(script);
    	System.out.println(keyBag.findKeyFromPubHash(addressBytes)) ;
    ...
    public RedeemData getConnectedRedeemData(KeyBag keyBag) throws ScriptException {
        ...
        } else if(connectedScript.isSentToChecksumContract()) {
            byte[] addressBytes = connectedScript.getPubKeyHash();
            return RedeemData.of(keyBag.findKeyFromPubHash(addressBytes), connectedScript);
        } else if(connectedScript.isTimeLockChecksumTx()) {    	
        	byte[] addressBytes = connectedScript.getPubKeyHash();
        	return RedeemData.of(keyBag.findKeyFromPubHash(addressBytes), connectedScript);
