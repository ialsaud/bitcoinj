...
public class ScriptBuilder {
	...
	public static Script createChecksumContractInputScript(@Nullable TransactionSignature signature, ECKey pubKey) {
		...
		return new ScriptBuilder().data(checksum1).data(sigBytes).data(pubkeyBytes).build();
	}
	...
	public static Script updateScriptWithSignature(Script scriptSig, byte[] signature, int targetIndex,
		...
		System.out.println(inputChunks.subList(sigsPrefixCount, totalChunks - sigsSuffixCount));
