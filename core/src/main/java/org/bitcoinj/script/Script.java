...
public class Script {
    ...
    public boolean isSentToChecksumContract() {
    	/*System.out.println(chunks.get(0).equalsOpCode(OP_DUP));
    	System.out.println(chunks.get(1).equalsOpCode(OP_HASH160));
    	System.out.println(chunks.get(2).data.length == Address.LENGTH);
    	System.out.println(chunks.get(3).equalsOpCode(OP_EQUALVERIFY));
    	System.out.println(chunks.get(4).equalsOpCode(OP_CHECKSIG));
    	System.out.println( chunks.get(5).equalsOpCode(OP_VERIFY));
    	System.out.println( chunks.get(6).data.length == 16);
    	System.out.println(chunks.get(7).equalsOpCode(OP_EQUAL));
    	System.out.println( chunks.get(6).data.length); */
    	return chunks.size() == 8 &&
                ...
                chunks.get(6).data.length == 16 && //TODO
    ...
    public boolean isTimeLockChecksumTx(){
    	return chunks.size()==19 &&
    			...
    			/* chunks.get(1).equalsOpCode(OP_DUP) &&
                 ...
                 chunks.get(5).equalsOpCode(OP_CHECKSIG) &&
                 chunks.get(6).equalsOpCode(OP_VERIFY) &&
                 //chunks.get(7).data.length == 16 &&
                 chunks.get(8).equalsOpCode(OP_EQUAL) &&
    			 chunks.get(9).equalsOpCode(OP_ELSE) &&
    			 chunks.get(10).equalsOpCode(OP_CHECKLOCKTIMEVERIFY) &&
    		     chunks.get(11).equalsOpCode(OP_DROP) &&
    		     chunks.get(12).equalsOpCode(OP_DUP) &&
                 chunks.get(13).equalsOpCode(OP_HASH160) &&
                 //chunks.get(3).data.length == Address.LENGTH &&
                 chunks.get(15).equalsOpCode(OP_EQUALVERIFY) &&
                 chunks.get(16).equalsOpCode(OP_CHECKSIG);   */ 	
    }
    /**
     ...
     *
     */
    public byte[] getPubKeyHash2(Script script) throws ScriptException {
    	//ScriptChunk a=chunks.get(10);
    ...
    public Script createEmptyInputScript(@Nullable ECKey key, @Nullable Script redeemScript) {
        if (isSentToAddress()) {
        	...
        	checkArgument(key != null, "Key required to create ChecksumContract input script");
            return ScriptBuilder.createChecksumContractInputScript(null, key);
        	...
        	checkArgument(key != null, "Key required to create TimeChecksumContract input script");
            return ScriptBuilder.createTimeLockContractInputScript(null, key,redeemScript);
        }
        else {
     ...
     * @return
     */
    /*public Script createEmptyInputScript(@Nullable ECKey key, @Nullable Script redeemScript, List<String> checksumArr) {
        if (isSentToAddress()) {
            checkArgument(key != null, "Key required to create pay-to-address input script");
            return ScriptBuilder.createInputScript(null, key);
        } else if (isSentToRawPubKey()) {
            ...
            return ScriptBuilder.createChecksumContractInputScript(null, key, checksumArr);
        } else {
            throw new ScriptException("Do not understand script type: " + this);
        }
    }*/
    ...
    public int getNumberOfBytesRequiredToSpend(@Nullable ECKey pubKey, @Nullable Script redeemScript) {
        if (isPayToScriptHash()) {
            ...
            int uncompressedPubKeySize = 65;
            int checksum = 16; /*this is an assumption TODO*/
            return checksum + SIG_SIZE + (pubKey != null ? pubKey.getPubKey().length : uncompressedPubKeySize);
        } else if (isTimeLockChecksumTx()) {
            int uncompressedPubKeySize = 65;
            int file = 256; //sha256 hash of the file portion
            return file + 5 + SIG_SIZE +(pubKey != null ? pubKey.getPubKey().length : uncompressedPubKeySize);
            ...
            if (chunk.opcode == OP_0) {
               ...
               // if (opcode > OP_16) {
                ...
                case OP_CHECKLOCKTIMEVERIFY:
                    ...
                    break;
                case OP_VERIFYLOCKTIME:
                    long timestamp = new Timestamp(System.currentTimeMillis()).getTime();
                    BigInteger l=new BigInteger(Long.toString(timestamp).substring(0, 10));
                    //System.out.println(l);
                	BigInteger k=castToBigInteger(script.chunks.get(0).data) ;
                	Boolean validtime=k.compareTo(l)>0;
                	stack.add(validtime ? new byte[] {1} : new byte[] {});
                	//if (k.compareTo(l) > 0 ){
                		//ifStack.add(true);
                	//}
                	//else{
    ...
    private static void executeCheckLockTimeVerify(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        //txContainingThis.setLockTime(1);;
        if (nLockTime.compareTo(BigInteger.ZERO) < 0)
            throw new ScriptException("Negative locktime");
        System.out.println((((txContainingThis.getLockTime() <  Transaction.LOCKTIME_THRESHOLD))));
